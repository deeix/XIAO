# XIAO Notify

Готовая связка из прошивки Seeed Studio XIAO nRF52840 и iOS-приложения. Модуль получает уведомления текущего iPhone через Apple Notification Center Service (ANCS) и отправляет их приложению по собственному BLE-сервису.

## Возможности

- история последних 1000 уведомлений и 1000 системных событий;
- фильтры «Все», OKX, Binance, Bybit, Simple и «Другое»;
- встроенные иконки известных приложений и цветные инициалы для остальных;
- обработка Added, Modified и Removed в выбираемом режиме;
- журнал подключения, потери связи, восстановления, ANCS и ошибок протокола;
- автоматическое переподключение и восстановление CoreBluetooth;
- сохранение истории между запусками без внешнего сервера.

## Прошивка XIAO nRF52840

Файл прошивки: `XIAO_ANCS_AppBridge/XIAO_ANCS_AppBridge.ino`.

Готовый проверенный образ: `XIAO_ANCS_AppBridge/firmware/XIAO-Notify-nRF52840.uf2`. Если serial DFU в Arduino IDE зависает, запустите `FLASH_XIAO.cmd`, дважды быстро нажмите RESET и дождитесь автоматического копирования образа на загрузочный диск XIAO.

1. Установите Arduino IDE и пакет плат Seeed nRF52 Boards.
2. Выберите `Seeed XIAO nRF52840` и библиотеку Adafruit Bluefruit nRF52 из пакета платы.
3. Откройте `.ino`, соберите и загрузите его на модуль.
4. В настройках Bluetooth iPhone выберите `XIAO Notify` и подтвердите сопряжение.
5. Разрешите модулю показывать уведомления, когда iOS предложит доступ ANCS.

Встроенный красный светодиод мигает, пока XIAO рекламируется и ждёт телефон. После BLE-подключения светодиод гаснет. Частое мигание означает, что BLE-реклама не смогла запуститься.

Сервис приложения использует UUID:

```text
Service: 6E400001-B5A3-F393-E0A9-E50E24DCCA9E
RX:      6E400002-B5A3-F393-E0A9-E50E24DCCA9E (write)
TX:      6E400003-B5A3-F393-E0A9-E50E24DCCA9E (notify)
```

Протокол версии 2 — UTF-8 JSON Lines. Один JSON завершается `\n` и может быть разделён на любое число BLE-пакетов. Типы сообщений: `notification`, `status`, `log`. Уведомления содержат `session_id` и `seq`; приложение подтверждает сохранение командой `ack`, а модуль повторяет неподтверждённый кадр. Чтение пакетов v1 сохранено для совместимости. Неизвестные поля игнорируются.

## Установка iOS-приложения

1. Запустите GitHub Actions workflow `Build iOS IPA`.
2. Скачайте артефакт `XIAO-Notify-unsigned-ipa`.
3. Подпишите и установите `XIAO-Notify.ipa` через SideStore или AltStore.
4. Разрешите Bluetooth, откройте «Настройки» → «Найти XIAO Notify» и выберите модуль.

После первого соединения приложение запоминает устройство. Зелёный индикатор означает, что BLE-канал и ANCS готовы. Нажатие на баннер соединения открывает подробный статус модуля; открытый экран запрашивает статус и RSSI каждые три секунды и отмечает данные устаревшими через десять секунд без ответа.

## Важные ограничения iOS

ANCS передаёт Bundle ID, имя, категорию, заголовок и текст, но не иконку приложения и не исходное время создания уведомления. Приложение пытается получить иконку по Bundle ID через best-effort private API, а если iOS не отдаёт её, использует встроенные иконки известных приложений или инициалы. Временем записи считается момент получения.

Приложение не создаёт повторный системный баннер: исходное уведомление iOS уже было показано пользователю.
